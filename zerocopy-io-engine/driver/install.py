...
#ITR = 5000	# interrupt throttling rate
...
execute('lsmod | grep "^ixgbe" > /dev/null && sudo rmmod -f ixgbe')
execute('lsmod | grep ps_ixgbe > /dev/null && sudo rmmod -f ps_ixgbe')
...
for i in range(num_ifs):
	...
	#execute('ifconfig %s promisc' % ifname)
...
execute('rm -f /dev/packet_shader_ixgbe')
execute('mknod /dev/packet_shader_ixgbe c 1011 0')
execute('chmod 666 /dev/packet_shader_ixgbe')
