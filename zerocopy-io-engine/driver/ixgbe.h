...
#if 0
#if defined(CONFIG_DCA) ||fined(CONFIG_DCA_MODULE)
#define IXGBE_DCA   
#include <linux/dca.h>
#endif
#endif

#define ZERO_COPY_IOENGINE
...
#define IXGBE_MAX_TXD		1024
#define IXGBE_MIN_TXD		64

#define IXGBE_DEFAULT_RXD   2048	
#define IXGBE_MAX_RXD		2048
...
#define IXGBE_MAX_SUBWINDOWS	(IXGBE_MAX_RXD / IXGBE_SUBWINDOW_SIZE)

#ifdef ZERO_COPY_IOENGINE
#define IXGBE_MAX_FREE_BUF_COUNT    (IXGBE_MAX_RXD << 1)
#define IXGBE_FREE_BUF_COUNT_MASK   (IXGBE_MAX_FREE_BUF_COUNT - 1)
#define IXGBE_DEFAULT_RXD_MASK      (IXGBE_DEFAULT_RXD - 1)
#define IXGBE_QIDX_BITS             (8)
#define IXGBE_RECYCLE_BATCH         (32)
#define IXGBE_RECYCLE_THRESH        (48)
#undef  IXGBE_MAX_SUBWINDOWS
#define IXGBE_MAX_SUBWINDOWS	    (IXGBE_MAX_FREE_BUF_COUNT >> IXGBE_SUBWINDOW_BITS)
#define IXGBE_DEFAULT_BIDX_BITS     (15)
#endif
...
#define IXGBE_TXD_BATCH (64)
#define IXGBE_RXD_BATCH (64)
...
union desc_map_type {
    uint32_t index;
    struct {
        uint32_t bidx:15; //interface index
        uint32_t qidx:8;  //queue index
        uint32_t ifidx:9; //buffer index
    }dmi;
};
...
#ifdef ZERO_COPY_IOENGINE
    union desc_map_type* desc_map_idx;//[IXGBE_MAX_RXD];  //descriptor map free buffer
    uint32_t* free_buf_idx;//[IXGBE_MAX_F/REE_BUF_COUNT];  //free buffer index array
    u16 fbi_next_to_read;  //free buffer index next to read, get free buffer
    u16 fbi_next_to_write; //free buffer index next to write, fill the free buffer to free_buf_idx
    u16 free_buf_count;
    FIFO_BUFFER* recycle_buf_fifo __attribute((aligned(64)));//[MAX_DEVICE];
    FIFO_CTRL* recycle_control_fifo;//[MAX_DEVICE];
#endif
	spinlock_t lock;
