...
#define ADAPTER_BITS	(6)
#define ADAPTER_BIT_MASK	((1 << ADAPTER_BITS) -1)
                               ...
                               int *work_done, int work_to_do)
{
    ...
    int rx_ifidx, rx_qidx, j, tx_ifidx;
    struct ixgbe_adapter* rx_adapter;
...
#ifdef ZERO_COPY_IOENGINE
    prefetchnta(tx_ring->desc_map_idx + i);
#endif
...
#ifdef ZERO_COPY_IOENGINE
    if(-1 == *work_done && count > 0) {
        for(j = 0; j < adapters_found; j++) //find the index of tx interface
            if(adapter == adapters[j])
                break;

        if(j >= adapters_found) { // can not find
            printk("ERRER: ixgbe_clean_tx_irq can not find the tx adapters\n");
            return count > 0;
        }
        ...
        for(j = 0; j < count; j++) {
            if(0xf == (j & 0xf))
                prefetchnta(tx_ring->desc_map_idx + i + 1);

            rx_ifidx = tx_ring->desc_map_idx[i].dmi.ifidx;
            rx_qidx = tx_ring->desc_map_idx[i].dmi.qidx;
            elem.index = tx_ring->desc_map_idx[i].dmi.bidx;

            //if(elem.index >= IXGBE_MAX_FREE_BUF_COUNT)
            //    printk("recycle ifidx %d , qidx %d, bidx %d\n", rx_ifidx, rx_qidx, elem.index);

            if(rx_ifidx > adapters_found) {
                printk("ixgbe_clean_tx_irq, ifidx(%d) > adapters_found(%d)\n",
                        rx_ifidx, adapters_found);
                goto recycle_next;
            }
            rx_adapter = adapters[rx_ifidx];

            if(rx_qidx > adapter->num_rx_queues) {
                printk("ixgbe_clean_tx_irq, qidx(%d) > adapter->num_rx_queues(%d)\n",
                        rx_qidx, adapter->num_rx_queues);
                goto recycle_next;
            }
            rx_ring = &rx_adapter->rx_ring[rx_qidx];

            /*printk("before:\nfifo[%d].batchSize = %d, read = %d, write = %d\nlocalWrite = %d, nextRead = %d, rBatch = %d\nlocalRead = %d, nextWrite = %d, wBatch = %d\n",
                tx_ifidx,
                rx_ring->recycle_control_fifo[tx_ifidx].batchSize,
                rx_ring->recycle_control_fifo[tx_ifidx].read,
                rx_ring->recycle_control_fifo[tx_ifidx].write,
                rx_ring->recycle_control_fifo[tx_ifidx].localWrite,
                rx_ring->recycle_control_fifo[tx_ifidx].nextRead,
                rx_ring->recycle_control_fifo[tx_ifidx].rBatch,
                rx_ring->recycle_control_fifo[tx_ifidx].localRead,
                rx_ring->recycle_control_fifo[tx_ifidx].nextWrite,
                rx_ring->recycle_control_fifo[tx_ifidx].wBatch
                );
            */

            insert(&rx_ring->recycle_control_fifo[tx_ifidx], &rx_ring->recycle_buf_fifo[tx_ifidx], elem);
            ...
            /*printk("after:\nfifo[%d].batchSize = %d, read = %d, write = %d\nlocalWrite = %d, nextRead = %d, rBatch = %d\nlocalRead = %d, nextWrite = %d, wBatch = %d\n",
                ...
                */
recycle_next:
            i = (i + 1) & (IXGBE_DEFAULT_TXD -1);
            if(!i)
                prefetchnta(tx_ring->desc_map_idx);
        }
    }
#endif
                                   ...
                                   int cleaned_count)
{
    ...
    __le64 addr;
#ifdef ZERO_COPY_IOENGINE
	unsigned int j = rx_ring->fbi_next_to_read;
    uint16_t fbi = 0;
	int real_clean_count;
    //int k;
	if(cleaned_count > rx_ring->free_buf_count)
		cleaned_count = rx_ring->free_buf_count;
	real_clean_count = cleaned_count;
	/*printk("rx_ring->fbi_next_to_read = %d, clean_count = %d, free_buf_count = %d\n",
					j, cleaned_count, rx_ring->free_buf_count);
    for(k = 0; k < MAX_DEVICE; k ++)
	    printk("rx_ring->recycle_control_fifo[%d].batchSize = %d, read = %d, write = %d\nlocalWrite = %d, nextRead = %d, rBatch = %d\nlocalRead = %d, nextWrite = %d, wBatch = %d\n",
					k,
                    rx_ring->recycle_control_fifo[k].batchSize,
                    rx_ring->recycle_control_fifo[k].read,
                    rx_ring->recycle_control_fifo[k].write,
                    rx_ring->recycle_control_fifo[k].localWrite,
                    rx_ring->recycle_control_fifo[k].nextRead,
                    rx_ring->recycle_control_fifo[k].rBatch,
                    rx_ring->recycle_control_fifo[k].localRead,
                    rx_ring->recycle_control_fifo[k].nextWrite,
                    rx_ring->recycle_control_fifo[k].nextWrite
                    );
    */
#endif
	while (cleaned_count--) {
		...
		addr = cpu_to_le64(packet_dma(rx_ring, i));
#else
        fbi = rx_ring->free_buf_idx[j];
		addr = cpu_to_le64(packet_dma(rx_ring, fbi));
        rx_ring->desc_map_idx[i].index = fbi;
		//if(rx_ring->desc_map_idx[i] == -1)
		//	printk("desc_map_idx[%d] = %d, free_buf_idx[%d] = %d\n",
		//					i, rx_ring->desc_map_idx[i], j, rx_ring->free_buf_idx[j]);
		j = (j + 1) & (IXGBE_MAX_FREE_BUF_COUNT - 1);
#endif
	...
	if (rx_ring->next_to_use != i) {
#ifdef ZERO_COPY_IOENGINE
        if((IXGBE_MAX_RXD - 1)  == real_clean_count || (IXGBE_DEFAULT_RXD - 1) == real_clean_count){
		    __le64 addr = cpu_to_le64(packet_dma(rx_ring, j));
            rx_ring->desc_map_idx[i].index = rx_ring->free_buf_idx[j];
            rx_desc ++;
            j++;
		    rx_desc->read.pkt_addr = addr;
		    rx_desc->read.hdr_addr = 0;
        ...
        rx_ring->fbi_next_to_read = j;
		rx_ring->free_buf_count -= real_clean_count;
#endif
		...
		int num_to_xmit, struct ps_pkt_info *info, char *buf)
{
...
#ifdef ZERO_COPY_IOENGINE
    struct ixgbe_ring *src_ring[MAX_DEVICE];
    unsigned char src_ifindex, src_qidx;
    int bidx;
    u64 dma_buf;

    for(i = 0; i < adapters_found; i++)
        src_ring[i] = adapters[i]->rx_ring;
#endif
    ...
    if((char*)2 == buf || left < num_to_xmit) {
        int tmp = 0;
#ifdef ZERO_COPY_IOENGINE
        if((char*)1 == buf || (char*)2 == buf)
            tmp = -1;
#endif
    ...
    cnt = ((qidx + cnt) & 0xfffffffc) - qidx;

#ifndef ZERO_COPY_IOENGINE	
	dst = packet_buf(tx_ring, qidx);
#endif
	//tx_desc = IXGBE_TX_DESC_ADV(*tx_ring, qidx);
	//prefetchnta(tx_desc + 0);
	//prefetchnta(tx_desc + 4);

	//prefetchnta(&tx_ring->tx_buffer_info[qidx + 0]);
	//prefetchnta(&tx_ring->tx_buffer_info[qidx + 4]);

	//prefetchnta(&tx_ring->desc_map_idx[qidx + 0]);
	//prefetchnta(&tx_ring->tx_buffer_info[qidx + 16]);
	...
	for (i = 0; i < cnt; i++) {
        ...
        /*if(0 == (qidx & 0x03)) {
	        ...
	        prefetchnta(tx_desc + 8);
        }*/

		next_qidx = (qidx + 1) & (tx_ring->count - 1);

        //if(0 == (i & 0xf))
	    //    prefetchnta(&tx_ring->tx_buffer_info[qidx + 32]);
...
#else
        bidx = info[i].dmi.bidx;
        src_qidx = info[i].dmi.qidx;
        src_ifindex = info[i].dmi.ifidx;
		//if(src_qidx >= adapter->num_rx_queues || src_ifindex >= adapters_found || bidx >= IXGBE_MAX_FREE_BUF_COUNT ) {
		if(src_qidx >= 1 || src_ifindex >= 2 || bidx >= IXGBE_MAX_FREE_BUF_COUNT ) {
            printk("drop the invalidate pkt qidx %d, ifidx %d bidx %d\n", src_qidx, src_ifindex, bidx);
			continue;
        }

        dma_buf = packet_dma(&src_ring[src_ifindex][src_qidx], bidx);
        tx_ring->desc_map_idx[qidx].index = info[i].index;
#endif
		tx_desc = IXGBE_TX_DESC_ADV(*tx_ring, qidx);
...
#ifdef ZERO_COPY_IOENGINE
	if(work_to_do > rx_ring->free_buf_count)
		work_to_do = rx_ring->free_buf_count;
#endif
...
#ifdef ZERO_COPY_IOENGINE
	memset(rx_ring->desc_map_idx, (IXGBE_MAX_RXD - 1), IXGBE_MAX_RXD * sizeof(uint32_t));

	rx_ring->fbi_next_to_read = 0;
	for(i = 0; i < IXGBE_MAX_FREE_BUF_COUNT; i++)
		rx_ring->free_buf_idx[i] = i;	
	rx_ring->fbi_next_to_write = 0;
    rx_ring->free_buf_count = i;

	memset(rx_ring->recycle_buf_fifo, 0, MAX_DEVICE * sizeof(FIFO_BUFFER));

	memset(rx_ring->recycle_control_fifo, 0, MAX_DEVICE * sizeof(FIFO_CTRL));
	for(i = 0; i < MAX_DEVICE; i++){
        rx_ring->recycle_control_fifo[i].batchSize = BATCH_SIZE;
        rx_ring->recycle_control_fifo[i].rbatchSize = IXGBE_RECYCLE_BATCH;
    }
#endif
...
#endif
#ifdef CONFIG_IXGBE_NAPI
		poll = &ixgbe_clean_rxtx_many;  // the real path
#endif
	} else {
		num_q_vectors = 1;
#ifndef IXGBE_NO_LRO
...
#ifdef ZERO_COPY_IOENGINE
	tx_ring->recycle_buf_fifo = NULL;	
	tx_ring->recycle_control_fifo = NULL;
	tx_ring->free_buf_idx = NULL;
    tx_ring->fbi_next_to_read = 0;
	tx_ring->fbi_next_to_write = 0;
    tx_ring->free_buf_count = 0;

	tx_ring->desc_map_idx = vmalloc_node(IXGBE_MAX_RXD * sizeof(uint32_t), adapter->numa_node);
	if(!tx_ring->desc_map_idx) {
		DPRINTK(PROBE, ERR,
		        "Unable to vmalloc buffer memory for "
		        "the desc map idx ring\n");
		goto alloc_dmi_failed;
	}
	memset(tx_ring->desc_map_idx, -1, IXGBE_MAX_TXD * sizeof(uint32_t));
#endif
                             ...
                             struct ixgbe_ring *rx_ring)
{
...
#ifdef ZERO_COPY_IOENGINE
	rx_ring->recycle_buf_fifo = vmalloc_node(MAX_DEVICE * sizeof(FIFO_BUFFER), adapter->numa_node);
	if (!rx_ring->recycle_buf_fifo) {
		DPRINTK(PROBE, ERR,
		        "Unable to vmalloc buffer memory for "
		        "the recycle buffer fifo\n");
		goto alloc_rb_fifo_failed;
	}
    memset(rx_ring->recycle_buf_fifo, 0, MAX_DEVICE * sizeof(FIFO_BUFFER));
	
	rx_ring->recycle_control_fifo = vmalloc_node(MAX_DEVICE * sizeof(FIFO_CTRL), adapter->numa_node);
	if(!rx_ring->recycle_control_fifo) {
		DPRINTK(PROBE, ERR,
		        "Unable to vmalloc buffer memory for "
		        "the recycle control fifo\n");
		goto alloc_rc_fifo_failed;
	}
    memset(rx_ring->recycle_control_fifo, 0, MAX_DEVICE * sizeof(FIFO_CTRL));
    for(i = 0; i < MAX_DEVICE; i++) {
        rx_ring->recycle_control_fifo[i].batchSize = BATCH_SIZE;
	...
	rx_ring->desc_map_idx = vmalloc_node(IXGBE_MAX_RXD * sizeof(uint32_t), adapter->numa_node);
	if(!rx_ring->desc_map_idx) {
		DPRINTK(PROBE, ERR,
		        "Unable to vmalloc buffer memory for "
		        "the desc map idx ring\n");
		goto alloc_dmi_failed;
	...
	rx_ring->free_buf_idx = vmalloc_node(IXGBE_MAX_FREE_BUF_COUNT * sizeof(uint32_t), adapter->numa_node);
	if(!rx_ring->free_buf_idx) {
		DPRINTK(PROBE, ERR,
		        "Unable to vmalloc buffer memory for "
		        "the desc map idx ring\n");
		goto alloc_fbi_failed;
	}
    rx_ring->fbi_next_to_read = 0;
	for(i = 0; i < IXGBE_MAX_FREE_BUF_COUNT; i++)
		rx_ring->free_buf_idx[i] = i;
...
#ifdef ZERO_COPY_IOENGINE
	for (i = 0; i <= (rx_ring->count * 2 - 1) / IXGBE_SUBWINDOW_SIZE; i++) {
#else
	for (i = 0; i <= rx_ring->count / IXGBE_SUBWINDOW_SIZE; i++) {
#endif
...
#ifdef ZERO_COPY_IOENGINE
alloc_fbi_failed:
	vfree(rx_ring->desc_map_idx);
	rx_ring->desc_map_idx = NULL;
alloc_dmi_failed:
	vfree(rx_ring->recycle_control_fifo);
	rx_ring->recycle_control_fifo = NULL;
alloc_rc_fifo_failed:
	vfree(rx_ring->recycle_buf_fifo);
	rx_ring->recycle_buf_fifo = NULL;
alloc_rb_fifo_failed:
                             ...
                             struct ixgbe_ring *rx_ring)
{
	...
	vfree(rx_ring->free_buf_idx);
	rx_ring->free_buf_idx = NULL;
    rx_ring->fbi_next_to_read = 0;
    rx_ring->fbi_next_to_write = 0;
    rx_ring->free_buf_count = 0;
...
#else
		size = PAGE_ALIGN(MAX_PACKET_SIZE * IXGBE_MAX_FREE_BUF_COUNT);
#endif
...
int ps_mmap(struct file *filp, struct vm_area_struct *vma)
{
...
#ifdef ZERO_COPY_IOENGINE    
    unsigned char qidx = vma->vm_pgoff & ((1 << IXGBE_QIDX_BITS) - 1);
    unsigned char ifindex =  (vma->vm_pgoff >> IXGBE_QIDX_BITS) & ADAPTER_BIT_MASK;
    struct ixgbe_adapter *adapter; 
    struct ixgbe_ring *rx_ring;
    ...
    if(ifindex >= adapters_found) {
        printk("ps_mmap() failed: the ifindex is larger than the adapters_found\n");
        return -EINVAL;
    }
    adapter = adapters[ifindex];
    if(qidx >= adapter->num_rx_queues || qidx >= adapter->num_tx_queues) {
        printk("ps_mmap() failed: the qidx is larger than the num_rx/tx_queues\n");
        return -EINVAL;
    }
    rx_ring = &adapter->rx_ring[qidx];
#endif
...
#else
    bidx = 0; //buffer index
    buf = packet_buf(rx_ring, bidx);
	for (up = vma->vm_start; up < vma->vm_end; up += PAGE_SIZE) {
        kp = packet_buf(rx_ring, bidx);
		bidx += 2;
		page = virt_to_page(kp);
		SetPageReserved(page);
		get_page(page);
		err = vm_insert_page(vma, up, page);
		if (err){
			printk("ps_mmap() vm_inset_page error, page address = %lx\n",(unsigned long)page);
			break;
		}
	}
#endif
	if (err) {
		...
		vfree(buf);
#else
			ClearPageReserved(virt_to_page(kp));
#endif
...
next:
		...
		rx_desc->read.pkt_addr = cpu_to_le64(packet_dma(rx_ring, qidx));
        rx_desc->read.hdr_addr = 0;
...
u16 round_robin_recycle_free_buf(FIFO_BUFFER *fifo_buf, FIFO_CTRL *fifo_ctrl, uint32_t *free_buf_idx, u16* fbi_next_to_write)
{
    ELEMENT_TYPE fbi;
    //u16 recycle_count = 0, recycle = 0;
    u16 recycle_count = 0;
    int i, j;
    u16 next_to_write = *fbi_next_to_write;
    //uint32_t rbatchSize = fifo_ctrl->rbatchSize;


    for(i = 0; i < adapters_found; i++) {
        //recycle = 0;
        //for(j = 0; j < IXGBE_RECYCLE_BATCH; j++) {
        //for(j = 0; j < rbatchSize; j ++) {
        for(j = 0; ; j ++) {
            if(SUCCESS != extract(&fifo_ctrl[i], &fifo_buf[i], &fbi))
                break;
            free_buf_idx[next_to_write] = fbi.index;
            if(fbi.index >= IXGBE_MAX_FREE_BUF_COUNT)
                printk("recycle fbi.index = %d\n", fbi.index);
            //recycle++;
            recycle_count ++;
            next_to_write = (next_to_write + 1) & (IXGBE_MAX_FREE_BUF_COUNT - 1); 
        }

        /*recycle_count += recycle;

        fifo_ctrl[i].rbatchSize = (recycle == 0 ? 1 : recycle); //record the real payload

        if(recycle > 0 && j >= rbatchSize) { //tune the rbatchSize dynamically
            rbatchSize = rbatchSize << 3;
            fifo_ctrl[i].rbatchSize = rbatchSize;

            if(rbatchSize > IXGBE_MAX_RXD) 
                fifo_ctrl[i].rbatchSize = IXGBE_MAX_RXD; 
        }*/
    }
    *fbi_next_to_write = next_to_write;

    return recycle_count;
}

int get_pkt_idx(struct ixgbe_ring* rx_ring, int to_get_cnt, struct ps_pkt_info *info)
{
#define PROBE_BATCH (7)
	struct ixgbe_adapter *adapter = rx_ring->adapter;
	union ixgbe_adv_rx_desc *rx_desc;
	...
	int qidx = rx_ring->next_to_clean;
    static int probe = PROBE_BATCH;
    static char probe_flag = 0;
	int next_qidx;
	int cnt = 0;
    int fbi_next_to_read;
    uint32_t bfidx;
	...
	rx_desc = IXGBE_RX_DESC_ADV(*rx_ring, (((qidx & 0xfffffffc) + probe) & (IXGBE_DEFAULT_RXD_MASK)));
	staterr = le32_to_cpu(rx_desc->wb.upper.status_error);
	if (!(staterr & IXGBE_RXD_STAT_DD))
		return cnt;
	...
	prefetchnta(IXGBE_RX_DESC_ADV(*rx_ring, ((qidx + 4) & (rx_ring->count - 1))));
	//prefetcht0(info);
    //prefetchnta(&rx_ring->desc_map_idx[qidx]);

    fbi_next_to_read = rx_ring->fbi_next_to_read;
    //prefetchnta(rx_ring->free_buf_idx + fbi_next_to_read);
	total_rx_packets = 0;
	total_rx_bytes = 0;
	
	while (cnt < to_get_cnt) {
        if((qidx & 3) == 0) {
	        rx_desc = IXGBE_RX_DESC_ADV(*rx_ring, ((qidx + 3) & (rx_ring->count -1)));
		    staterr = le32_to_cpu(rx_desc->wb.upper.status_error);
		    if (!(staterr & IXGBE_RXD_STAT_DD))
			    break;
        }
	    rx_desc = IXGBE_RX_DESC_ADV(*rx_ring, qidx);
        //if((cnt & 7) == 7)
	    //    prefetcht0(info + cnt + 1);
            
		staterr = le32_to_cpu(rx_desc->wb.upper.status_error);

		next_qidx = (qidx + 1) & (IXGBE_DEFAULT_RXD_MASK);
        if((qidx & 3) == 3) {
            //prefetchnta(&rx_ring->desc_map_idx[next_qidx]);
	        prefetchnta(IXGBE_RX_DESC_ADV(*rx_ring, next_qidx + 4));
        }
		...
		*/
        ...
        //if((fbi_next_to_read & 0x07) == 0)
        //    prefetchnta(rx_ring->free_buf_idx + fbi_next_to_read + 16);
		...
		info[cnt].len = len;
        info[cnt].index = rx_ring->desc_map_idx[qidx].index;
        ...
        /* prefetch packet data for user to read packet  */
	    prefetchnta(packet_buf(rx_ring, rx_ring->desc_map_idx[qidx].index));
        
        bfidx = rx_ring->free_buf_idx[fbi_next_to_read];
        fbi_next_to_read = (fbi_next_to_read + 1) & (IXGBE_FREE_BUF_COUNT_MASK);

        if(bfidx >= IXGBE_MAX_FREE_BUF_COUNT) {
            printk("bfidx = %d, fbi_next_to_read = %d\n", bfidx, fbi_next_to_read);
            bfidx &= (IXGBE_FREE_BUF_COUNT_MASK);
        }

        rx_desc->read.pkt_addr = cpu_to_le64(packet_dma(rx_ring, bfidx));
        rx_desc->read.hdr_addr = 0;
        rx_ring->desc_map_idx[qidx].index = bfidx;

		qidx = next_qidx;
        continue;

gpi_next:
        bfidx = rx_ring->desc_map_idx[qidx].index;
        if(bfidx > IXGBE_MAX_FREE_BUF_COUNT) {
            printk("desc_map_idx[%d] = %d\n", qidx, bfidx);
            bfidx &= (IXGBE_FREE_BUF_COUNT_MASK);
        }
		rx_desc->read.pkt_addr = cpu_to_le64(packet_dma(rx_ring, bfidx));
        rx_desc->read.hdr_addr = 0;
	...
	if (cnt > 0) {
        rx_ring->fbi_next_to_read = fbi_next_to_read;
        rx_ring->free_buf_count -= cnt;
		rx_ring->queued = qidx;
		rx_ring->next_to_clean = qidx;
		rx_ring->next_to_use = 
				(qidx == 0) ? (rx_ring->count - 1) : (qidx - 1);
...
found:
	...
	*/
...
#else
    if(chunk.queue.ifindex >= adapters_found) {
        printk("ps_recv_chunk() chunk.queue.ifindex is larger than adapters_found\n");
        return -EINVAL;
    }

    adapter = adapters[chunk.queue.ifindex];
    if(chunk.queue.qidx >= adapter->num_rx_queues) {
        printk("ps_recv_chunk() chunk.queue.qidx is larger than adapter->num_rx_queues\n");
        return -EINVAL;
    }
	...
	retcnt = chunk.retcnt;	
	front = chunk.front;
	if(retcnt > 0) { //rx recycle mode
		if(front < retcnt) {
			first_idx = MAX_CHUNK_SIZE + front - retcnt;
			left = MAX_CHUNK_SIZE - first_idx;
			tmp = copy_from_user(context->info, chunk.info + first_idx, 
										left * pkt_info_size );
			if(!tmp)
				tmp = copy_from_user(context->info + left, chunk.info,
										(retcnt - left) * pkt_info_size );
		}
		else {
			first_idx = front - retcnt;
			tmp = copy_from_user(context->info, chunk.info + first_idx,
										retcnt * pkt_info_size);
		}

		if(tmp) {
			printk("ps_recv_chunk copy chunk.info error !\n");
			return -EFAULT;
		}

	    next_to_write = rx_ring->fbi_next_to_write;
		for(i = 0; i < retcnt; i++) {
			rx_ring->free_buf_idx[next_to_write] = context->info[i].dmi.bidx;
            next_to_write = (next_to_write + 1) & (IXGBE_MAX_FREE_BUF_COUNT - 1);
        }
        rx_ring->fbi_next_to_write = next_to_write;
		rx_ring->free_buf_count += retcnt;
	}
    

    if(!retcnt && rx_ring->free_buf_count < chunk.cnt) {//tx recycle mode
        rx_ring->free_buf_count += round_robin_recycle_free_buf(rx_ring->recycle_buf_fifo, 
                                                                rx_ring->recycle_control_fifo,
                                                                rx_ring->free_buf_idx,
                                                                &rx_ring->fbi_next_to_write);
    }

    cnt = ((chunk.cnt < rx_ring->free_buf_count) ? chunk.cnt : rx_ring->free_buf_count);

    if(cnt == 0) { // there is no packet buffers 
        printk("buffer is %d\n", cnt);
	    goto ZERO_COPY_IOENGINE_FOUND;
    }

ZERO_COPY_IOENGINE_RETRY:
	if (chunk.recv_blocking)
		prepare_to_wait(&context->wq, &wait, TASK_INTERRUPTIBLE);

    ret = get_pkt_idx(rx_ring, cnt, context->info);
	...
	if (chunk.recv_blocking) {
		...
		goto ZERO_COPY_IOENGINE_RETRY;
	} 
    else
		return -EWOULDBLOCK;

ZERO_COPY_IOENGINE_FOUND:
    if(ret > 0) {
        if(ret + front < MAX_CHUNK_SIZE)
	        tmp = copy_to_user(chunk.info + front, context->info, 
			        			ret * pkt_info_size);
        else {
            left = MAX_CHUNK_SIZE - chunk.front;

            tmp = copy_to_user(chunk.info + front, context->info,
                                left * pkt_info_size);
            if(!tmp) {
                tmp = copy_to_user(chunk.info, context->info + left, 
                                (ret - left) * pkt_info_size);
            }
        }
    }
...
#define RECYCLE_BATCH   (16)
    ...
    */
    ...
    /*if(chunk.cnt > IXGBE_TXD_BATCH)
        cnt = IXGBE_TXD_BATCH;
    else*/
        cnt = chunk.cnt & 0x7fff;
    
    eidx = chunk.front + cnt;

    if(eidx >= MAX_CHUNK_SIZE){
        int count = MAX_CHUNK_SIZE - chunk.front;

	    ret = copy_from_user(context->info, chunk.info + chunk.front,
		    	count * sizeof(struct ps_pkt_info));
        if(!ret)
	        ret = copy_from_user(context->info + count, chunk.info,
		    	     (cnt - count) * sizeof(struct ps_pkt_info));
    }
    else
	    ret = copy_from_user(context->info, chunk.info + chunk.front,
			    cnt * sizeof(struct ps_pkt_info));
	if (ret) {
	...
	ret = ixgbe_xmit_batch(tx_ring, cnt, context->info, kbuf);
#endif
