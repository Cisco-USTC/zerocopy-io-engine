...
void handle_signal(int signal)
{
	...
	for (i = 0; i < num_devices_attached; i++) {
		total_tx_packets += handle->tx_packets[my_cpu];
		total_rx_packets += handle->rx_packets[my_cpu];
	}
	...
	for (i = 0; i < num_devices_attached; i++) {
		...
		printf("RX %ld packets "
				...
				handle->rx_packets[my_cpu] / 
				  (double)handle->rx_chunks[my_cpu]);

		printf("TX %ld packets "
				"(%ld chunks, %.2f packets per chunk)\n", 
				handle->tx_packets[my_cpu],
				handle->tx_chunks[my_cpu],
				handle->tx_packets[my_cpu] / 
				  (double)handle->tx_chunks[my_cpu]);
	}
...
void echo()
{
	...
	for (;;) {
		...
		if (!sink) {
            ...
            for(i = 0; i < ret; i ++) {
                chunk.info[front + i].dmi.ifidx = chunk.queue.ifindex;
                chunk.info[front + i].dmi.qidx = chunk.queue.qidx;
            }
