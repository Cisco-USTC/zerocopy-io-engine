...
void update_stats(struct ps_handle *handle)
{
	...
	if (!first) {
		...
		printf("CPU %d: %6.4lf Mpps, %6.4f Gbps "
				"(%-3.2f packets per chunk)",
				my_cpu, 
				pps / 1000000.0,
		...
		int num_flows)
{
	...
	chunk.queue.ifindex = devices_registered[0];
	...
	while (1) {
		int working = 0;
        int left, count, eidx;
        int start = (chunk.front + chunk.cnt) & (MAX_CHUNK_SIZE -1);

        left = MAX_CHUNK_SIZE - chunk.cnt;
        count = (left > chunk_size) ? chunk_size : 0;
        /*
         * if(left > chunk_size)
         *     count = chunk_size;
         * else if(left > CACHE_LINE_SIZE / sizeof(ps_pkt_info)
         *     count = CACHE_LINE_SIZE / sizeof(ps_pkt_info);
         * else
         *     count = 0;
         */
        if(chunk.cnt > chunk_size*2)
            count = 0;
        eidx = start + count;
		...
		for (i = 0; i < num_devices_registered; i++) {
			...
			for (j = start; j < eidx; ) {
				...
				chunk.info[j].offset = j * MAX_PACKET_SIZE;
#else
				chunk.info[j].index = j;
#endif
				...
				if (num_flows == 0){
					if (ip_version == 4){
						build_packet(chunk.buf + chunk.info[j].index * MAX_PACKET_SIZE,
							     packet_size, &seed);
					} else {
						build_packet_v6(chunk.buf + chunk.info[j].index * MAX_PACKET_SIZE,
							packet_size, &seed);
					}
				}
				else
					memcpy_aligned(chunk.buf + chunk.info[j].index * MAX_PACKET_SIZE, 
						packet[(next_flow[i] + j) % num_flows], 
						packet_size);
                //if(chunk.info[j].dmi.bidx != chunk.info[j].index)
                //    printf("dmi.bidx = %x, index = %x\n", chunk.info[j].dmi.bidx,chunk.info[j].index);
				chunk.info[j].dmi.ifidx = chunk.queue.ifindex;
				chunk.info[j].dmi.qidx = my_cpu;
#endif
                j++;
                if(MAX_CHUNK_SIZE == j) {
                    eidx -= MAX_CHUNK_SIZE;
